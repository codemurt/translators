...
class Tse:
    ...
    @staticmethod
    def get_client_session(http_client: str = 'aiohttp', proxies: Optional[dict] = None) -> SessionType:
        if http_client not in ('aiohttp', 'httpx'):
            raise TranslatorError
    ...
    @staticmethod
    def get_response_status(response: ResponseType, http_client: str = "aiohttp"):
        return response.status if http_client == 'aiohttp' else response.status_code

    @staticmethod
    def get_response_cookies(response: ResponseType, http_client: str = 'aiohttp'):
        if http_client == 'aiohttp':
            return {key: morsel.value for key, morsel in response.cookies.items()}
        return dict(response.cookies)

    @staticmethod
    async def get_response_text(response: ResponseType, http_client: str = 'aiohttp'):
        return await response.text() if http_client == 'aiohttp' else response.text

    @staticmethod
    async def get_response_json(response: ResponseType, http_client: str = "aiohttp"):
        return await response.json(content_type=None) if http_client == 'aiohttp' else response.json()

    @staticmethod
    def if_session_exists(session: Optional[SessionType]):
        if session is None:
            return False
        
        if isinstance(session, SessionType):
            if isinstance(session, aiohttp.ClientSession) and session.closed:
                return False
            elif isinstance(session, httpx.AsyncClient) and session.is_closed:
                return False
        return True
...
class GoogleV1(Tse):
    ...
    async def get_language_map(self, host_html: str, **kwargs: LangMapKwargsType) -> dict:
    ...
    @Tse.time_stat
    @Tse.check_query
    async def google_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        :param **kwargs:
                :param session: Optinal[SessionType], default None.
        ...
        sleep_seconds = kwargs.get('sleep_seconds', 0)
        http_client = kwargs.get('http_client', 'aiohttp')
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.api_url):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)

            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(host_html, self.session, timeout, **debug_lang_kwargs)
            ...
            self.api_url = (
                '/translate_a/single?client={0}&sl={1}&tl={2}&hl=zh-CN'
                '&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t'
                '&ie=UTF-8&oe=UTF-8&source=bh&ssel=0&tsel=0&kc=1&tk={3}&q={4}'
            ).format('webapp', from_language, to_language, tk, urllib.parse.quote(query_text))  # [t,webapp]

        response = await self.session.get(self.api_url, headers=self.host_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
...
class GoogleV2(Tse):
    ...
    async def google_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)

            if urllib.parse.urlparse(self.consent_url).hostname == urllib.parse.urlparse(str(response.url)).hostname:
                form_data = self.get_consent_data(host_html)
                response = await self.session.post(self.consent_url, data=form_data, headers=self.host_headers, timeout=timeout)
                host_html = await Tse.get_response_text(response, http_client)

            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(host_html, **debug_lang_kwargs)

        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)

        rpc_data = self.get_rpc(query_text, from_language, to_language)
        rpc_data = urllib.parse.urlencode(rpc_data)
        response = await self.session.post(self.api_url, headers=self.api_headers, data=rpc_data, timeout=timeout)
        response.raise_for_status()
        response_text = await Tse.get_response_text(response, http_client)
        json_data = json.loads(response_text[6:])
...
class BaiduV1(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def baidu_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)  # must twice, send cookies.
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_lang_url, self.session, self.host_headers, timeout, **debug_lang_kwargs)
        ...
        response = await self.session.post(self.api_url, data=payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class BaiduV2(Tse):
    ...
    async def get_sign(self, query_text: str, host_html: str, http_client: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> str:
        gtk_list = re.compile("""window.gtk = '(.*?)';|window.gtk = "(.*?)";""").findall(host_html)[0]
        gtk = gtk_list[0] or gtk_list[1]

        response = await ss.get(self.get_sign_url, headers=headers, timeout=timeout)
        sign_html = await Tse.get_response_text(response, http_client)
    ...
    async def baidu_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.token and self.sign):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)  # must twice, reload token.
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            self.token = self.get_tk(host_html)
            self.sign = await self.get_sign(query_text, host_html, http_client, self.session, self.host_headers, timeout)
        ...
        response = await self.session.post(self.langdetect_url, headers=self.api_headers, data=payload, timeout=timeout)
        if from_language == 'auto':
            lang_data = await Tse.get_response_json(response, http_client)
            from_language = lang_data['lan']
        ...
        response = await self.session.post(self.api_url, params=params, data=payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class YoudaoV1(Tse):
    ...
    async def get_sign_key(self, host_html: str, http_client: str, ss: SessionType, timeout: Optional[float]) -> str:
        try:
            if not self.get_sign_url:
                self.get_sign_url = re.compile(self.get_sign_pattern).search(host_html).group()
            response = await ss.get(self.get_sign_url, headers=self.host_headers, timeout=timeout)
            response.raise_for_status()
        except:
            response = await ss.get(self.get_sign_old_url, headers=self.host_headers, timeout=timeout)
            response.raise_for_status()

        sign_html = await Tse.get_response_text(response, http_client)
        sign = re.compile('md5\\("fanyideskweb" \\+ e \\+ i \\+ "(.*?)"\\)').findall(sign_html)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def youdao_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.sign_key):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            self.sign_key = await self.get_sign_key(host_html, http_client, self.session, timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.language_url, self.session, self.host_headers, timeout, **debug_lang_kwargs)

        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)

        form = self.get_form(query_text, from_language, to_language, self.sign_key)
        response = await self.session.post(self.api_url, data=form, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class YoudaoV2(Tse):
    ...
    async def youdao_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.secret_key):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            _ = await self.session.get(self.login_url, headers=self.host_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.language_url, self.session, self.host_headers, timeout, **debug_lang_kwargs)

            self.get_js_url = re.compile(self.get_js_pattern).search(host_html).group()
            response = await self.session.get(self.get_js_url, headers=self.host_headers, timeout=timeout)
            js_html = await Tse.get_response_text(response, http_client)
            ...
            key_response = await self.session.get(self.get_key_url, params=params, headers=self.host_headers, timeout=timeout)
            key_data = await Tse.get_response_json(key_response, http_client)
            self.secret_key = key_data['data']['secretKey']
            self.decode_key = key_data['data']['aesKey'] or self.decode_key
            self.decode_iv = key_data['data']['aesIv'] or self.decode_iv
        ...
        response = await self.session.post(self.api_url, data=payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        result = await Tse.get_response_text(response, http_client)
        data = self.decrypt_result(text=result, key=self.decode_key, iv=self.decode_iv)
...
class QQFanyi(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, language_url: str, ss: SessionType, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(language_url, headers=self.host_headers, timeout=timeout)
        response.raise_for_status()
        js_html = await Tse.get_response_text(response, http_client)
        lang_map_str = re.compile('C={(.*?)}|languagePair = {(.*?)}', flags=re.S).search(js_html).group()  # C=
        return exejs.evaluate(lang_map_str)

    async def get_qt(self, http_client: str, ss: SessionType, timeout: Optional[float]) -> dict:
        response = await ss.post(self.get_qt_url, headers=self.qt_headers, json=self.qtv_qtk, timeout=timeout)
        data = await Tse.get_response_json(response, http_client)
        return data
    ...
    @Tse.time_stat
    @Tse.check_query
    async def qqFanyi_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        https://fanyi.qq.com
        :param query_text: str, must.
        :param from_language: str, default 'auto'.
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.qtv_qtk):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            self.qtv_qtk = await self.get_qt(http_client, self.session, timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_language_url, self.session, timeout, **debug_lang_kwargs)
        ...
        response = await self.session.post(self.api_url, headers=self.api_headers, data=payload, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class QQTranSmart(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=self.host_headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def qqTranSmart_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_lang_url, self.session, timeout, **debug_lang_kwargs)
        ...
        response = await self.session.post(self.api_url, json=split_payload, headers=self.api_headers, timeout=timeout)
        split_data = await Tse.get_response_json(response, http_client)
        ...
        response = await self.session.post(self.api_url, json=api_payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class AlibabaV1(Tse):
    ...
    def get_dmtrack_pageid(self, host_response: ResponseType, http_client: str) -> str:
        try:
            e = re.compile("dmtrack_pageid='(\\w+)';").findall(host_response.text)[0]
        except:
            e = ''
        if not e:
            d = Tse.get_response_cookies(host_response, http_client)
            e = d.get('cna', '001')
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, ss: SessionType, lang_url: str, use_domain: str, dmtrack_pageid: str, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        params = {'dmtrack_pageid': dmtrack_pageid, 'biz_type': use_domain}
        response = await ss.get(lang_url, params=params, headers=self.host_headers, timeout=timeout)
        language_dict = await Tse.get_response_json(response, http_client)
        return dict(map(lambda x: x, [(x['sourceLuange'], x['targetLanguages']) for x in language_dict['languageMap']]))

    @Tse.time_stat
    @Tse.check_query
    async def alibaba_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.dmtrack_pageid):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            host_response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            self.dmtrack_pageid = self.get_dmtrack_pageid(host_response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.session, self.get_language_url, use_domain, self.dmtrack_pageid, timeout, **debug_lang_kwargs)
        ...
        response = await self.session.post(self.api_url, headers=self.api_headers, params=params, data=payload, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class AlibabaV2(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, lang_html: str, **kwargs: LangMapKwargsType) -> dict:
    ...
    async def alibaba_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.csrf_token):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            self.get_language_url = f"https:{re.compile(self.get_language_pattern).search(host_html).group()}"
            response = await self.session.get(self.get_language_url, headers=self.host_headers, timeout=timeout)
            lang_html = await Tse.get_response_text(response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(lang_html, **debug_lang_kwargs)
            self.detail_language_map = self.get_d_lang_map(lang_html)

            _ = await self.session.get(self.csrf_url, headers=self.host_headers, timeout=timeout)
            response = await self.session.get(self.csrf_url, headers=self.host_headers, timeout=timeout)
            self.csrf_token = await Tse.get_response_json(response, http_client)
        ...
        if http_client == 'aiohttp':
            ...
            for field_name, (file_name, file_object) in files_data.items():
                form_data.add_field(name=field_name, value=file_object, filename=file_name)

            response = await self.session.post(self.api_url, data=form_data, headers=self.api_headers, timeout=timeout)
        else:  # httpx
            response = await self.session.post(self.api_url, files=files_data, headers=self.api_headers, timeout=timeout)
...
class Bing(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def bing_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.tk and self.ig_iid):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
        ...
        response = await self.session.post(api_url, headers=self.host_headers, data=payload, timeout=timeout)
        response.raise_for_status()
        time.sleep(sleep_seconds)
        self.query_count += 1
        ...
        except requests.exceptions.JSONDecodeError:  #122 fr
            data_html = await Tse.get_response_text(response, http_client)
...
class Sogou(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, host_html: str, lang_old_url: str, ss: SessionType, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        try:
            ...
            response = await ss.get(self.get_language_url, headers=self.host_headers, timeout=timeout)
        except:
            response = await ss.get(lang_old_url, headers=self.host_headers, timeout=timeout)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def sogou_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.uuid):
            self.uuid = str(uuid.uuid4())
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, host_html, self.get_language_old_url, self.session, timeout, **debug_lang_kwargs)
...
class Caiyun(Tse):
    ...
    # @Tse.debug_language_map
    # async def get_language_map(self, js_html: str, **kwargs: LangMapKwargsType) -> dict:
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict,  timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        lang_dict = await Tse.get_response_json(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def caiyun_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.tk and self.jwt):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            # js_url_path = re.compile(self.get_js_pattern).search(host_html).group()
            # self.get_js_url = ''.join([self.host_url, js_url_path])
            # js_html = await self.session.get(self.get_js_url, headers=self.host_headers, timeout=timeout).text
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_language_url, self.session, self.api_headers, timeout, **debug_lang_kwargs)

            jwt_payload = {'browser_id': self.browser_id}
            jwt_response = await self.session.post(self.get_jwt_url, json=jwt_payload, headers=self.api_headers, timeout=timeout)
            jwt_data = await Tse.get_response_json(jwt_response, http_client)
            self.jwt = jwt_data['jwt']
        ...
        response = await self.session.post(self.api_url, headers=self.api_headers, json=payload, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Deepl(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def deepl_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        https://www.deepl.com
        :param query_text: str, must.
        :param from_language: str, default 'auto'.
        ...
        r_s = await self.session.post(self.api_url, params=self.params['split'], json=ssp_data, headers=self.api_headers, timeout=timeout)
        r_s.raise_for_status()
        s_data = await Tse.get_response_json(r_s, http_client)
        ...
        r_cs = await self.session.post(self.api_url, params=self.params['handle'], json=h_data, headers=self.api_headers, timeout=timeout)
        r_cs.raise_for_status()
        data = await Tse.get_response_json(r_cs, http_client)
        time.sleep(sleep_seconds)
        self.request_id += 3
        self.query_count += 1
...
class YandexV1(Tse):
    ...
    async def detect_language(self, http_client: str, ss: SessionType, query_text: str, sid: str, yu: str, headers: dict, timeout: Optional[float]) -> str:
        params = {
            'sid': sid,
            'yu': yu,
            'text': query_text,
        ...
        response = await ss.get(self.detect_language_url, params=params, headers=headers, timeout=timeout)
        lang_data = await Tse.get_response_json(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def yandex_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.sid and self.yu):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.home_url, headers=self.host_headers, timeout=timeout)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
        ...
        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)
        if from_language == 'auto':
            from_language = await self.detect_language(http_client, self.session, query_text, self.sid, self.yu, self.api_headers, timeout)
...
class YandexV2(Tse):
    ...
    async def get_request_data(self, http_client: str, ss: SessionType, method: str, params: dict, timeout: Optional[float]) -> dict:
        url = f'{self.api_url}/{method}'
        params = {**{'srv': self.srv}, **params}
        response = await ss.post(url=url, params=params, data=self.api_payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        return data

    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, ss: SessionType, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        lang_map = {}
        lang_data = await self.get_request_data(http_client, ss=ss, method='getLangs', params={}, timeout=timeout)
    ...
    async def yandex_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, ss=self.session, timeout=timeout, **debug_lang_kwargs)
            if not self.language_map.get('zh'):
                self.language_map.update(self.add_zh_lang_map)

        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)
        if from_language == 'auto':
            lang_data = await self.get_request_data(http_client, ss=self.session, method='detect', params={'text': query_text}, timeout=timeout)
            from_language = lang_data.get('lang')
        ...
        data = await self.get_request_data(http_client, ss=self.session, method='translate', params=params, timeout=timeout)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Argos(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        lang_list = await Tse.get_response_json(response, http_client)
        lang_list = sorted([lang['code'] for lang in lang_list])
        return {}.fromkeys(lang_list, lang_list)

    async def get_secret(self, http_client: str, secret_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> str:
        response = await ss.get(secret_url, headers=headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def argos_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.secret):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(
                self.host_url, headers=self.host_headers, timeout=timeout
            )
            self.secret = await self.get_secret(http_client, self.secret_url, self.session, self.host_headers, timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.language_url, self.session, self.language_headers, timeout, **debug_lang_kwargs)
...
class Iciba(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, api_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        params = {'c': 'trans', 'm': 'getLanguage', 'q': 0, 'type': 'en', 'str': ''}
        response = await ss.get(api_url, params=params, headers=headers, timeout=timeout)
        # try:
        #     dd = await Tse.get_response_json(response, http_client)
        # except:
        #     dd_text = await Tse.get_response_text(response, http_client)
        #     dd = json.loads(dd_text)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def iciba_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.api_url, self.session, self.language_headers, timeout, **debug_lang_kwargs)
...
class IflytekV1(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, host_html: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        try:
        ...
        js_html = await Tse.get_response_text(r, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def iflytek_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
            _ = await self.session.get(self.cookies_url, headers=self.host_headers, timeout=timeout)
            _ = await self.session.get(self.info_url, headers=self.host_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, host_html, self.session, self.host_headers, timeout, **debug_lang_kwargs)
...
class IflytekV2(Tse):
    ...
    async def get_language_map(self, http_client: str, host_html: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        ...
        response = await ss.get(host_js_url, headers=headers, timeout=timeout)
        host_js_html = await Tse.get_response_text(response, http_client)
        self.language_url = f"""{host_true_url}{re.compile(self.language_url_pattern).search(host_js_html).group()}"""

        response = await ss.get(self.language_url, headers=headers, timeout=timeout)
        lang_js_html = await Tse.get_response_text(response, http_client)
    ...
    async def iflytek_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        if from_language == 'auto':
            ...
            detect_response = await self.session.get(self.detect_language_url, params=params, headers=self.host_headers, timeout=timeout)
            status_code = Tse.get_response_status(detect_response, http_client)
            r_text = await Tse.get_response_text(detect_response, http_client)
            r_data = await Tse.get_response_json(detect_response, http_client)
            from_language = r_data['data'] if status_code == 200 and r_text.strip() != '' else self.output_zh
        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)
...
class Iflyrec(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, lang_index: dict, **kwargs: LangMapKwargsType) -> dict:
    ...
    @Tse.time_stat
    @Tse.check_query
    async def iflyrec_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(self.lang_index, **debug_lang_kwargs)

        if from_language == 'auto':
            params = {'t': self.get_timestamp()}
            form = {'originalText': query_text}
            detect_response = await self.session.post(self.detect_lang_url, params=params, json=form, headers=self.api_headers, timeout=timeout)
            detect_data = await Tse.get_response_json(detect_response, http_client)
            from_language_id = detect_data['biz'][0]['detectionLanguage']
        ...
        response = await self.session.post(self.api_url, params=api_params, json=api_form, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Reverso(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def reverso_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.decrypt_language_map):
            ...
            response = await self.session.get(self.language_url, headers=self.host_headers, timeout=timeout)
            lang_html = await Tse.get_response_text(response, http_client)
        ...
        response = await self.session.post(self.api_url, json=payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Itranslate(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def itranslate_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            if not self.language_url:
                response = await self.session.get(self.manifest_url, headers=self.host_headers, timeout=timeout)
                manifest_data = await Tse.get_response_json(response, http_client)
...
class TranslateCom(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, lang_desc: dict, **kwargs: LangMapKwargsType) -> dict:
        return {
            item["code"]: [it["code"] for it in item["availableTranslationLanguages"]]
            for item in lang_desc
        }

    @Tse.time_stat
    @Tse.check_query
    async def translateCom_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            lang_response = await self.session.get(self.language_url, headers=self.host_headers, timeout=timeout)
            self.language_description = await Tse.get_response_json(lang_response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(self.language_description, **debug_lang_kwargs)

        if from_language == 'auto':
            detect_form = {'text_to_translate': query_text}
            r_detect = await self.session.post(self.lang_detect_url, data=detect_form, headers=self.api_headers, timeout=timeout)
            detect_data = await Tse.get_response_json(r_detect, http_client)
...
class Utibet(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def utibet_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'ti', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
...
class Papago(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def papago_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.auth_key):
        ...
        if from_language == 'auto':
            ...
            r_detect = await self.session.post(self.lang_detect_url, headers=detect_headers, data=detect_form, timeout=timeout)
            detect_data = await Tse.get_response_json(r_detect, http_client)
            from_language = detect_data['langCode']
        ...
        payload = urllib.parse.urlencode(payload)
        response = await self.session.post(self.api_url, headers=trans_headers, data=payload, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class LingvanexV1(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        ...
        response = await ss.get(lang_url, params=params, headers=headers, timeout=timeout)
        detail_lang_map = await Tse.get_response_json(response, http_client)
        for _ in range(3):
            _ = await ss.get(lang_url, params={'platform': 'dp'}, headers=headers, timeout=timeout)
        lang_list = sorted(set([item['full_code'] for item in detail_lang_map['result']]))
        return {}.fromkeys(lang_list, lang_list)

    async def get_d_lang_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> dict:
        params = {'all': 'true', 'code': 'en_GB', 'platform': 'dp', '_': self.get_timestamp()}
        response = await ss.get(lang_url, params=params, headers=headers, timeout=timeout)
        data = await Tse.get_response_json(response, http_client)
        return data

    async def get_auth(self, http_client: str, auth_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> dict:
        response = await ss.get(auth_url, headers=headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
        data = {k: v for k, v in re.compile(',(.*?)="(.*?)"').findall(js_html)}
        return data

    @Tse.time_stat
    @Tse.check_query
    async def lingvanex_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.auth_info and self.mode == mode):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            self.auth_info = await self.get_auth(http_client, self.auth_url, self.session, self.host_headers, timeout)
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.language_url, self.session, self.host_headers, timeout, **debug_lang_kwargs)
            self.detail_language_map = await self.get_d_lang_map(http_client, self.language_url, self.session, self.host_headers, timeout)
...
class LingvanexV2(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        self.detail_language_map = await Tse.get_response_json(response, http_client)
    ...
    async def lingvanex_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.auth):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            host_html = await Tse.get_response_text(response, http_client)
...
class NiutransV1(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def niutrans_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.account_info and self.api_headers):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            _ = self.session.options(self.cookie_url, headers=self.host_headers, timeout=timeout)

            response = await self.session.get(self.user_url, headers=self.host_headers, timeout=timeout)
            user_data = await Tse.get_response_json(response, http_client)
            response = await self.session.get(self.key_url, headers=self.host_headers, timeout=timeout)
            key_data = await Tse.get_response_json(response, http_client)
            guest_info = {
            ...
            r_tk = await self.session.post(self.token_url, json=guest_info, headers=self.host_headers, timeout=timeout)
            token_data = await Tse.get_response_json(r_tk, http_client)
        ...
        from_language, to_language = self.check_language(from_language, to_language, self.language_map, output_zh=self.output_zh)
        if from_language == 'auto':
            res = await self.session.post(self.detect_language_url, json={'src_text': query_text}, headers=self.api_headers, timeout=timeout)
            res_data = await Tse.get_response_json(res, http_client)
            from_language = res_data['data']['language']
...
class NiutransV2(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        d_lang_map = await Tse.get_response_json(response, http_client)
        lang_list = sorted(set([it['code'] for item in d_lang_map['languageList'] for it in item['result']]))
        return {}.fromkeys(lang_list, lang_list)

    async def get_captcha_id(self, http_client: str, captcha_url: str, ss: SessionType, headers: dict, timeout: Optional[float]):
        response = await ss.get(captcha_url, headers=headers, timeout=timeout)
        captcha_host_html = await Tse.get_response_text(response, http_client)
        ...
        response = await ss.get(captcha_js_url, headers=headers, timeout=timeout)
        captcha_js_html = await Tse.get_response_text(response, http_client)
    ...
    async def get_geetest_data(self, http_client: str, timeout: Optional[float]):
        gl_params = {
        ...
        r_gl = await self.session.get(self.geetest_load_url, params=gl_params, headers=self.host_headers, timeout=timeout)
        r_gl_text = await Tse.get_response_text(r_gl, http_client)
        self.geetest_load_data = json.loads(r_gl_text[22:-1])['data']
        ...
        r_gv = await self.session.get(self.geetest_verify_url, params=gv_params, headers=self.host_headers, timeout=timeout)
        r_gv_text = await Tse.get_response_text(r_gv, http_client)
        self.geetest_verify_data = json.loads(r_gv_text[22:-1])['data']['seccode']
        return
    ...
    async def niutrans_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.captcha_id):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            _ = await self.session.get(self.login_url, headers=self.host_headers, timeout=timeout)
            self.captcha_id = await self.get_captcha_id(http_client, self.geetest_captcaha_url, self.session, self.host_headers, timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_language_url, self.session, self.api_headers, timeout, **debug_lang_kwargs)
        ...
        if from_language == 'auto':
            ...
            res = await self.session.get(self.detect_language_url, params=params, headers=self.host_headers, timeout=timeout)
            res_data = await Tse.get_response_json(res, http_client)
            from_language = res_data['language']

        await self.get_geetest_data(http_client, timeout)
        trans_params = {
            'src_text': query_text,
            'from': from_language,
            'to': to_language,
        ...
        response = await self.session.get(self.api_url, params=trans_params, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Mglip(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def mglip_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'mon', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
...
class VolcEngine(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def volcEngine_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        response = await self.session.post(self.api_url, params=params, json=payload, headers=self.api_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class ModernMt(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def modernMt_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
...
class MyMemory(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, http_client: str, host_html: str, lang_url: str, ss: SessionType, headers: dict,
        ...
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        lang_d_list = await Tse.get_response_json(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def myMemory_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            host_html = await Tse.get_response_text(response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, host_html, self.get_matecat_language_url, self.session,
        ...
        response = await self.session.get(api_url, params=params, headers=self.host_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Mirai(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def mirai_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'ja', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        not_update_cond_freq = 1 if self.query_count % update_session_after_freq != 0 else 0
        not_update_cond_time = 1 if time.time() - self.begin_time < update_session_after_seconds else 0
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time and self.tran_key):
            self.begin_time = time.time()
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            # _ = await self.session.get(self.home_url, headers=self.host_headers, timeout=timeout)
            response = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.lang_url, self.session, self.api_json_headers, timeout, **debug_lang_kwargs)

        if from_language == 'auto':
            response = await self.session.post(self.detect_lang_url, headers=self.api_json_headers, json={'text': query_text}, timeout=timeout)
            res_data = await Tse.get_response_json(response, http_client)
            from_language = res_data['language']
        ...
        _ = await self.session.post(self.trace_url, json=trace_data, headers=self.api_text_headers, timeout=timeout)
        ...
        response = await self.session.post(self.api_url, data=payload, headers=self.api_text_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Apertium(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        ...
        data = {f_lang: [v for k, v in lang_pairs if k == f_lang] for f_lang, t_lang in lang_pairs}
        return data

    @Tse.time_stat
    @Tse.check_query
    async def apertium_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if from_language == 'auto':
            ...
            response = await self.session.post(self.detect_lang_url, data=payload, headers=self.api_headers, timeout=timeout)
            langs = await Tse.get_response_json(response, http_client)
...
class Tilde(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, sys_data: dict, **kwargs: LangMapKwargsType) -> dict:
    ...
    @Tse.time_stat
    @Tse.check_query
    async def tilde_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            response = await self.session.get(self.get_config_url, headers=self.host_headers, timeout=timeout)
            self.config_data = await Tse.get_response_json(response, http_client)
            ...
            response = await self.session.get(sys_url, params=params, headers=self.api_headers, timeout=timeout)
            self.sys_data = await Tse.get_response_json(response, http_client)  # test
            self.langpair_ids = self.get_langpair_ids(self.sys_data)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(self.sys_data, **debug_lang_kwargs)
...
class cloudTranslationV1(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, d_lang_map: dict, **kwargs: LangMapKwargsType) -> dict:
    ...
    @Tse.time_stat
    @Tse.check_query
    async def cloudTranslation_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            _ = await self.session.get(self.get_cookie_url, headers=self.api_headers, timeout=timeout)
            response = await self.session.get(self.get_lang_url, headers=self.api_headers, timeout=timeout)
            d_lang_map = await Tse.get_response_json(response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(d_lang_map, **debug_lang_kwargs)
        ...
        if from_language == 'auto':
            ...
            response = await self.session.post(self.detect_lang_url, json=payload, headers=self.api_headers, timeout=timeout)
            res_data = await Tse.get_response_json(response, http_client)
            from_language = res_data['data']['language']
...
class SysTran(Tse):
    ...
    async def get_client_data(self, http_client: str, client_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> dict:
        response = await ss.get(client_url, headers=headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def sysTran_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            self.session = kwargs.get('session', None) or Tse.get_client_session(http_client, proxies)
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            self.client_data = await self.get_client_data(http_client, self.get_client_url, self.session, self.host_headers, timeout)
            payload = urllib.parse.urlencode(self.client_data)
            response = await self.session.post(self.get_token_url, data=payload, headers=self.api_ajax_headers, timeout=timeout)
            self.token_data = await Tse.get_response_json(response, http_client)
            ...
            response = await self.session.get(self.get_lang_url, headers=self.api_json_headers, timeout=timeout)
            d_lang_map = await Tse.get_response_json(response, http_client)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(d_lang_map, **debug_lang_kwargs)
            self.professional_field = self.get_professional_field_list(d_lang_map)
        ...
        response = await self.session.post(self.api_url, json=payload, headers=self.api_json_headers, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class TranslateMe(Tse):
    ...
    # @Tse.time_stat
    # @Tse.check_query
    async def _translateMe_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
    ...
    @Tse.time_stat
    @Tse.check_query
    async def translateMe_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        https://translateme.network/
        :param query_text: str, must.
        :param from_language: str, default 'auto'.
        ...
        if self.output_en in (from_language, to_language):
            return await self._translateMe_api(query_text, from_language, to_language, **kwargs)

        tmp_kwargs = kwargs.copy()
        tmp_kwargs.update({'is_detail_result': False, 'if_show_time_stat': False})
        next_query_text = await self._translateMe_api(query_text, from_language, self.output_en, **tmp_kwargs)
        return await self._translateMe_api(next_query_text, self.output_en, to_language, **kwargs)
...
class Elia(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, dd: dict, **kwargs: LangMapKwargsType) -> dict:
    ...
    @Tse.time_stat
    @Tse.check_query
    async def elia_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if from_language == 'auto':
            ...
            response = await self.session.post(self.detect_lang_url, data=payload, headers=self.api_headers, timeout=timeout)
            res_data = await Tse.get_response_json(response, http_client)
            from_language = res_data['lang_id']
        from_language, to_language = self.check_language(from_language, to_language, self.language_map)
...
class LanguageWire(Tse):
    ...
    async def get_language_map(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float], **kwargs: LangMapKwargsType) -> dict:
        response = await ss.get(lang_url, headers=headers, timeout=timeout)
        d_lang_map = await Tse.get_response_json(response, http_client)
        data = {ii['sourceLanguage']['mmtCode']: [jj['targetLanguage']['mmtCode'] for jj in d_lang_map if jj['sourceLanguage']['mmtCode'] == ii['sourceLanguage']['mmtCode']] for ii in d_lang_map}
        return data

    # async def get_lwt_data(self, http_client: str, lwt_js_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> dict:
    #     response = await ss.get(lwt_js_url, headers=headers, timeout=timeout)
    #     js_html = await Tse.get_response_text(response, http_client)
    #     lwt_data = {
    ...
    @Tse.time_stat
    @Tse.check_query
    async def languageWire_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            _ = await self.session.post(self.cookie_url, headers=self.api_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.lang_url, self.session, self.api_headers, timeout, **debug_lang_kwargs)
...
class Judic(Tse):
    ...
    @Tse.debug_language_map
    async def get_language_map(self, lang_list: List[str], **kwargs: LangMapKwargsType) -> dict:
        return {}.fromkeys(lang_list, lang_list)

    @Tse.uncertified
    @Tse.time_stat
    @Tse.check_query
    async def judic_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            _ = await self.session.get(self.host_url, headers=self.host_headers, timeout=timeout)
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(self.lang_list, **debug_lang_kwargs)
...
class Yeekit(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def yeekit_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
...
class Hujiang(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def hujiang_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType,) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            update_cookies = {'HJ_UID': self.hj_uid, 'HJC_USRC': 'uzhi', 'HJC_NUID': '1'}
            if http_client == 'aiohttp':
                self.session.cookie_jar.update_cookies(update_cookies)
            else:
        ...
        response = await self.session.post(api_url, headers=self.api_headers, data=payload, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Xunjie(Tse):
    ...
    async def get_info(self, http_client: str, lang_url: str, ss: SessionType, headers: dict, timeout: Optional[float]) -> dict:
        params = {'version': self.version}
        response = await ss.get(lang_url, params=params, headers=headers, timeout=timeout)
        js_html = await Tse.get_response_text(response, http_client)
    ...
    @Tse.time_stat
    @Tse.check_query
    async def xunjie_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
        ...
        if not (Tse.if_session_exists(self.session) and self.language_map and not_update_cond_freq and not_update_cond_time):
            ...
            debug_lang_kwargs = self.debug_lang_kwargs(from_language, to_language, self.default_from_language, if_print_warning)
            self.language_map = await self.get_language_map(http_client, self.get_language_url, self.session, self.host_headers, timeout, **debug_lang_kwargs)
            # self.info_data = await self.get_info(http_client, self.get_info_url, self.session, self.host_headers, timeout)
            self.info_data = {
                'productId': 146,
        ...
        if from_language == 'auto':
            try:
                ...
                lang_response = await self.session.post(self.detect_lang_url, headers=self.api_headers, data=pay_data, timeout=timeout)
                lang_data = await Tse.get_response_json(lang_response, http_client)
                from_language = lang_data['getcode']
            except Exception as e:
                print(str(e))
        ...
        pay_data = urllib.parse.urlencode(payload)
        response = await self.session.post(self.api_url, headers=self.api_headers, data=pay_data, timeout=timeout)
        response.raise_for_status()
        data = await Tse.get_response_json(response, http_client)
        time.sleep(sleep_seconds)
        self.query_count += 1
...
class Lara(Tse):
    ...
    @Tse.time_stat
    @Tse.check_query
    async def lara_api(self, query_text: str, from_language: str = 'auto', to_language: str = 'en', **kwargs: ApiKwargsType) -> Union[str, dict]:
        """
...
class TranslatorsServer:
    ...
    async def translate_text(
        ...
        **kwargs: ApiKwargsType,
        ...
        :param **kwargs:
                :param is_detail_result: bool, default False.
                :param if_close_session: bool, default True.
                :param http_client: str, default 'aiohttp'. Union['aiohttp', 'httpx']
        ...
        result = await self.translators_dict[translator](query_text=query_text, from_language=from_language, to_language=to_language, **kwargs)
        if if_close_session:
            await self.close(translator=translator, if_print_warning=if_print_warning)
        return result
    ...
    async def translate_html(
        ...
        :param **kwargs:
                ...
                :param session: Optional[SessionType], default None.
                :param http_client: str, default 'aiohttp'. Union['aiohttp', 'httpx']
                :param professional_field: str, support alibaba(), baidu(), caiyun(), cloudTranslation(), elia(), sysTran(), youdao(), volcEngine() only.
                :param session: Optinal[SessionType], default None.
                :param timeout: Optional[float], default None.
        ...
        async def _translate_sentence(sentence: str) -> Tuple[str, str]:
            async with asyncio.Semaphore(n_jobs):
                result = await self.translate_text(
                    ...
                    if_close_session=False,  # must False
                    **kwargs,
                )
        ...
        result_list = await asyncio.gather(*[_translate_sentence(sentence) for sentence in sentence_list])
        await self.close(translator=translator, if_print_warning=if_print_warning)
    ...
    async def _test_translate(self, _ts: str, timeout: Optional[float] = None, if_show_time_stat: bool = False) -> str:
    ...
    async def close(self, translator: Optional[str] = None, if_print_warning: bool = True):
        if translator is None:
            for ts in self._translators_dict.keys():
                await self._close(ts, if_print_warning)
        else:
            await self._close(translator, if_print_warning)
        return

    async def _close(self, translator: str, if_print_warning: bool = True):
        _translator = self._translators_dict[translator]
        session = _translator.session
        if isinstance(session, SessionType):
            if isinstance(session, aiohttp.ClientSession):
                if if_print_warning:
                    sys.stderr.write(f'{translator} aiohttp session closed.\n')
                if not session.closed:
                    await session.close()
            elif isinstance(session, httpx.AsyncClient):
                if if_print_warning:
                    sys.stderr.write(f'{translator} httpx session closed.\n')
    ...
    async def preaccelerate(self, timeout: Optional[float] = None, if_show_time_stat: bool = True, **kwargs: str) -> dict:
    ...
    async def speedtest(self, **kwargs: List[str]) -> None:
